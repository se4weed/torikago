...
class Bar::GemVersionQuery
  ...
  def jwt_result
    {
      ...
      "result" => "bar jwt #{jwt_version} blocks unverified payload access: #{jwt_blocks_unverified_payload?}"
    }
  end
  ...
  def postcode_result
...

    {
      ...
      "result" => "bar jpostcode #{Jpostcode::VERSION} returned #{address_result(address)}"
    }
  end
